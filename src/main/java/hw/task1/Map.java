...
public class Map extends JPanel {
        ...
        for (int y = 0; y < fieldSizeY; y++) {
            for (int x = 0; x < fieldSizeX; x++) {
                ...
                if (field[y][x] == HUMAN_DOT) {
                    ...
                    g.drawLine(x*cellWidth+DOT_PADDING,(y+1)*cellHeight-DOT_PADDING,(x+1)*cellWidth-DOT_PADDING,y*cellHeight+DOT_PADDING);
                } else if (field[y][x] == AI_DOT) {
                    g.setColor(Color.RED);
                    g.fillOval(x * cellWidth + DOT_PADDING, y * cellHeight + DOT_PADDING, cellWidth - DOT_PADDING * 2, cellHeight - DOT_PADDING * 2);
                } else {
                    throw new RuntimeException("Неверное значение " + field[y][x] + " в ячейке: x=" + x + " y=" + y);
                }
            }
        }
    ...
    private void showMessageGameOver(Graphics g) {
        ...
        switch (gameStateType) {
            ...
            case STATE_WIN_HUMAN -> g.drawString(MSG_WIN_HUMAN, 70, getHeight() / 2 + 60);
            default -> throw new RuntimeException("Неожиданное завершения игры " + gameStateType);
        }
        gameWork = false;
    }
